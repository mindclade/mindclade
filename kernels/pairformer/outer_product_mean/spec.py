                ...
                DimensionConstraint(
                    predicate=And(
                        operands=(
                            ...
                            Eq(lhs=DimRef(argument="left", axis=2), rhs=IntLiteral(value=32)),
                            Eq(lhs=DimRef(argument="left", axis=3), rhs=IntLiteral(value=64)),
                            Eq(lhs=DimRef(argument="right", axis=0), rhs=IntLiteral(value=1)),
                            Eq(lhs=DimRef(argument="right", axis=1), rhs=IntLiteral(value=64)),
                            Eq(lhs=DimRef(argument="right", axis=2), rhs=IntLiteral(value=32)),
                            Eq(lhs=DimRef(argument="right", axis=3), rhs=IntLiteral(value=64)),
                            Eq(lhs=DimRef(argument="mask", axis=0), rhs=IntLiteral(value=1)),
                            Eq(lhs=DimRef(argument="mask", axis=1), rhs=IntLiteral(value=64)),
                            Eq(lhs=DimRef(argument="mask", axis=2), rhs=IntLiteral(value=32)),
                        )
                    ),
                    code="EXACT_B1_S64_N32_C64",
                    message="requires exact [1,64,32,64] operands and [1,64,32] mask",
                ),
                DimensionConstraint(
                    predicate=IsFinite(
