...
STABLE_TORCH_LIBRARY(mindclade, m) {
  ...
  m.def("_pair_weighted_average_bwd(Tensor grad_output, Tensor value, Tensor weights, Tensor mask, float epsilon, Tensor output, Tensor lse, bool need_value_grad, bool need_weights_grad) -> (Tensor? grad_value, Tensor? grad_weights)");
  ...
  m.def("_transition_bwd(Tensor grad_output, Tensor gate, Tensor value, Tensor output_weight, Tensor output_bias, Tensor mask, Tensor pre_mask_output, bool need_gate_grad, bool need_value_grad, bool need_weight_grad, bool need_bias_grad, bool need_mask_grad) -> (Tensor? grad_gate, Tensor? grad_value, Tensor? grad_weight, Tensor? grad_bias, Tensor? grad_mask)");
