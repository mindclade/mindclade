...
def _mindclade_backward_fake_0(grad_output, left, right, mask, epsilon, output, normalizer, need_left_grad, need_right_grad, need_mask_grad):
    return (torch.empty_like(left), torch.empty_like(right), torch.empty_like(mask))
...
def _mindclade_backward_fake_1(grad_output, value, weights, mask, epsilon, output, lse, need_value_grad, need_weights_grad):
    return (torch.empty_like(value), torch.empty_like(weights))
...
def _mindclade_required_setup_context_1(ctx, inputs, output):
    ...
    ctx.set_materialize_grads(False)
    ctx.save_for_backward(inputs[0], inputs[1], inputs[2], output_values[0], output_values[1])
    ctx._mindclade_saved_scalars_1 = (inputs[3],)
...
def _mindclade_required_backward_1(ctx, *grad_outputs):
    ...
    saved_tensor_0, saved_tensor_1, saved_tensor_2, saved_tensor_3, saved_tensor_4 = ctx.saved_tensors
    saved_scalar_0, = ctx._mindclade_saved_scalars_1
    output_gradient_output = grad_outputs[0]
    if output_gradient_output is None:
...
def _mindclade_backward_fake_2(grad_output, gate, value, output_weight, output_bias, mask, pre_mask_output, need_gate_grad, need_value_grad, need_weight_grad, need_bias_grad, need_mask_grad):
    return (torch.empty_like(gate), torch.empty_like(value), torch.empty_like(output_weight), torch.empty_like(output_bias), torch.empty_like(mask))


def _mindclade_required_setup_context_2(ctx, inputs, output):
    output_values = output
    ctx.set_materialize_grads(False)
    ctx.save_for_backward(inputs[0], inputs[1], inputs[2], inputs[3], inputs[4], output_values[1])
...
def _mindclade_required_backward_2(ctx, *grad_outputs):
    ...
    saved_tensor_0, saved_tensor_1, saved_tensor_2, saved_tensor_3, saved_tensor_4, saved_tensor_5 = ctx.saved_tensors
    output_gradient_output = grad_outputs[0]
    if output_gradient_output is None:
