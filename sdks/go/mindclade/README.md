...
`github.com/mindclade/mindclade/sdks/go/mindclade` is the private
...
```go
import "github.com/mindclade/mindclade/sdks/go/mindclade"
```

Bazel consumers depend on `//sdks/go/mindclade:mindclade`. Run
`sdks/go/mindclade/scripts/bootstrap` once to resolve module
...
Durable work returns a `operationv1.Operation`. `OperationService` exposes one uniform
...
one reviewed raw-only method. `sdks/go/api.md` is the generated
per-RPC reference.
...
its layer order, so it reads the forward `sdks` to `libs` edge as
...
authority (`//sdks/go/mindclade:mindclade` and `:mindclade_test`). The
