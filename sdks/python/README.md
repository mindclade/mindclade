...
Every failure raised by an RPC call path is a `MindcladeError`. Three
configuration-, decode-, and constraint-time exceptions sit outside that
hierarchy and remain `ValueError` subclasses, so `except MindcladeError` does
**not** catch them; they are listed below and marked. The hierarchy is:
...
| `CrossFieldError` (a `ValueError`, **not** a `MindcladeError`) | One unsatisfied cross-field constraint, raised by the generated validators before a request leaves the facade. It carries `constraint`, `message_type`, `rule`, and `fields`, and names the same constraint identifier the server reports. |
