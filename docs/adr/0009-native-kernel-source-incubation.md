...
Permit the exact `kernels/native/` component and the operation-local TileLang source, Bazel ownership, documentation, and test files for `outer_product_mean`, `pair_weighted_average`, `triangle_attention`, and `triangle_multiplication` to exist before Wave 6 as a bounded, source-only incubation exception. The component lifecycle remains `proposed`, maturity remains `target`, `production_authority` remains `false`, and both qualified and active operation counts remain zero. Physical source presence and dispatcher schema declaration under this ADR are not activation.
...
- offline TileLang compiler intake that requires the exact four explicit operation-local sources and bounded profiles;
...
The committed native operator inventory contains exactly four declared, unqualified operations during incubation. Generated projections are reviewed build outputs from explicit repository inputs and must drift-check to that exact inventory. Incubation may compile schema-only code, reference implementations, and policy tests, but it cannot publish a native bundle, enable production dispatch, load a bundle in production, claim Stable ABI or GPU qualification, or treat a fake compiler test as TileLang/CUDA qualification.

This ADR is not JIT-06 ratification. Qualification or activation of any declared operator requires completed Wave 5 correctness, representative bottleneck evidence, an operation-local PyTorch reference and semantic owner, explicit forward/backward/boundary tolerances, supported hardware and toolchain locks, an operation-specific JIT-06 ADR, immutable signed bundle evidence, plan-bound dispatch, reference fallback, and tested revocation. Any fifth operator requires a separate amendment. The root Python lock and Bazel dependency graph must also gain matching Torch, pytest, TileLang, and native-toolchain authorities before their dependent targets can be called hermetic or release-qualified.
...
The package is intentionally unusable as a production provider while every declared operation remains unqualified and inactive. Bazel labels establish repository ownership but do not manufacture missing third-party dependency locks, GPU execution evidence, signatures, or connected qualification. Readiness reports must continue to say `TARGET`, zero active operations, zero qualified operations, and kernel K0 not achieved.
...
Source qualification checks the ADR/index, path-manifest closure, component lifecycle, exact Bazel labels, canonical four-operation native manifest, deterministic generation, strict namespace, schema consistency, loader policy, and generated architecture drift. Tests requiring undeclared or unlocked Torch, pytest, TileLang, CUDA, or Stable ABI dependencies remain explicit dependency gaps rather than passing evidence.

Connected and production qualification remain unavailable under this ADR. Any qualified or active operation without a separately ratified JIT-06 decision, production dispatch, unsigned or unverified bundle load, runtime discovery/compilation, alternate namespace, qualification claim, missing fallback, fifth operator, or expiry violation fails closed. Rollback removes the package and its manifest additions, regenerates Appendix A6 and the combined blueprint, preserves review evidence, and leaves operation-local PyTorch references as the only math authority.
