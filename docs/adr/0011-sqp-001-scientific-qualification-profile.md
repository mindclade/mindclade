...
PDB acquisition and dataset publication remain blocked until the PDB source-use approval contract is in state `approved` and binds the source terms, snapshot manifest, permitted purposes, retention/export controls, safety escalation, independent approvers, protected revision, and receipt digests. H100 execution remains blocked until the SQP-001 H100 approval contract is `approved` and binds the exact driver, CUDA, cuDNN, PyTorch, container/Nix closure, hardware, reservation, expiry, and maximum authorized cost.
