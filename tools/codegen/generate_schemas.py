...
def render_go_build() -> bytes:
    return (
        ...
        "go_library(\n"
        '    name = "bindings",\n'
        '    srcs = ["bindings.generated.go"],\n'
        '    importpath = "github.com/mindclade/mindclade/protocols/generated/go/schema/v1",\n'
        '    visibility = ["//visibility:public"],\n'
        '    deps = ["@com_github_santhosh_tekuri_jsonschema_v6//:jsonschema"],\n'
        ")\n\n"
        "go_test(\n"
        '    name = "bindings_test",\n'
        '    srcs = ["bindings_generated_test.go"],\n'
        '    embed = [":bindings"],\n'
        ")\n\n"
        "filegroup(\n"
        '    name = "generated_sources",\n'
        '    srcs = ["BUILD.bazel", "bindings.generated.go", "bindings_generated_test.go"],\n'
        '    visibility = ["//visibility:public"],\n'
        ")\n"
    ).encode("utf-8")
...
def render_rust(bindings: Sequence[SchemaBinding], fixtures: Mapping[str, Mapping[str, str]]) -> bytes:
    sources = schema_source_map(bindings)
    lines = [
        f"// Code generated by {GENERATOR}. DO NOT EDIT.",
        "",
    ...
    for binding in bindings:
        lines.extend(
            [
                ...
                f"pub fn decode_{binding.family}(content: &[u8]) -> Result<{binding.title}, SchemaError> {{",
    ...
    lines.extend(
        [
            ...
            "    match family {",
            *[
                f'        "{family}" => Some((r###"{value["positive"]}"###, r###"{value["negative"]}"###)),'
                for family, value in fixtures.items()
            ],
            "        _ => None,",
            "    }",
    ...
    lines.extend(
        [
            ...
            *[
            ...
            "}",
            "",
            f"const schemaText: Readonly<Record<SchemaFamily, string>> = {json.dumps(sources, sort_keys=True, indent=2)};",
            f"const fixtures: Readonly<Record<SchemaFamily, Readonly<{{ positive: string; negative: string }}>>> = {json.dumps(fixtures, sort_keys=True, indent=2)};",
            "",
...
def generated_binding_outputs(root: Path) -> dict[Path, bytes]:
    ...
    return {
        ...
        root / RUST_BINDING: render_rust(bindings, fixtures),
