...
func advanceOperation(ctx context.Context, tx *sql.Tx, identity Identity, operationID string, target *commonv1.ResourceRef, state jobv1.OperationState, at time.Time) error {
	...
	done := state == jobv1.OperationState_OPERATION_STATE_SUCCEEDED || state == jobv1.OperationState_OPERATION_STATE_FAILED || state == jobv1.OperationState_OPERATION_STATE_CANCELLED
