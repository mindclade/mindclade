...
func NewPageTokenCodec(key []byte) (*PageTokenCodec, error) {
	if len(key) < 32 {
		return nil, errors.New("workflow pagination HMAC key must contain at least 32 bytes")
	}
...
func (codec *PageTokenCodec) encode(value pageToken) (string, error) {
	if codec == nil || len(codec.key) < 32 {
		return "", errors.New("workflow pagination codec is not configured")
	}
	value.Version = 1
	...
	_, _ = signature.Write([]byte("mindclade.workflow-page.v1\x00"))
