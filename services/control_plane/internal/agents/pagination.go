...
func NewPageTokenCodec(key []byte) (*PageTokenCodec, error) {
	if len(key) < 32 {
		return nil, errors.New("agent pagination HMAC key must contain at least 32 bytes")
	}
...
func (codec *PageTokenCodec) encode(value pageToken) (string, error) {
	if codec == nil || len(codec.key) < 32 {
		return "", errors.New("agent pagination codec is not configured")
	}
	value.Version = 1
	...
	_, _ = signature.Write([]byte("mindclade.agent-page.v1\x00"))
...
func (codec *PageTokenCodec) decode(encoded string, expected pageToken) (pageToken, error) {
	...
	if err = decoder.Decode(&value); err != nil || value.Version != 1 {
		return pageToken{}, ErrInvalidArgument
	}
	if err = decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return pageToken{}, ErrInvalidArgument
	}
	if value.Kind != expected.Kind || value.Tenant != expected.Tenant || value.Project != expected.Project || value.Parent != expected.Parent || value.Filter != expected.Filter || value.Order != expected.Order {
		return pageToken{}, fmt.Errorf("%w: page token query binding mismatch", ErrInvalidArgument)
	}
	if value.AfterName == "" && value.AfterSequence == 0 {
		return pageToken{}, ErrInvalidArgument
	}
	return value, nil
}
