...
func (GeneratedEventFactory) PolicyCreated(identity Identity, value *policyv1.UsePolicy, operation *jobv1.Operation, command *commonv1.CommandContext, at time.Time) (*commonv1.EventEnvelope, error) {
	...
	return newEvent(identity, usePolicyResource(identity, value), payload, uint64(value.GetRevision()), command, at) //nolint:gosec // Conversion is bounded by validated protocol invariants or PostgreSQL CHECK constraints.
}
