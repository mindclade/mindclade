...
func NewPageTokenCodec(key []byte) (*PageTokenCodec, error) {
	if len(key) < 32 {
		return nil, errors.New("policy pagination HMAC key must contain at least 32 bytes")
	}
...
func (c *PageTokenCodec) encode(token pageToken) (string, error) {
	if c == nil || len(c.key) < 32 {
		return "", errors.New("policy pagination codec is not configured")
	}
	token.Version = 1
...
func (c *PageTokenCodec) decode(encoded string, expected pageToken) (pageToken, error) {
	...
	if token.Kind != expected.Kind || token.Tenant != expected.Tenant || token.Project != expected.Project || token.Filter != expected.Filter || token.Order != expected.Order {
		return pageToken{}, fmt.Errorf("%w: policy page token query binding mismatch", ErrInvalidArgument)
	}
	return token, nil
}
