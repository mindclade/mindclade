...
func NewPageTokenCodec(key []byte) (*PageTokenCodec, error) {
	if len(key) < 32 {
		return nil, errors.New("admin pagination HMAC key must contain at least 32 bytes")
	}
...
func (c *PageTokenCodec) encode(token pageToken) (string, error) {
	if c == nil || len(c.key) < 32 {
		return "", errors.New("admin pagination codec is not configured")
	}
	token.Version = 1
...
func (c *PageTokenCodec) decode(encoded string, expected pageToken) (pageToken, error) {
	...
	if err = decoder.Decode(&token); err != nil || token.Version != 1 || token.AfterID == "" {
		return pageToken{}, ErrInvalidArgument
	}
	if err = decoder.Decode(&struct{}{}); !errors.Is(err, io.EOF) {
		return pageToken{}, ErrInvalidArgument
	}
	if token.Kind != expected.Kind || token.Tenant != expected.Tenant || token.Project != expected.Project || token.Filter != expected.Filter || token.Order != expected.Order || token.QueryDigest != expected.QueryDigest {
		return pageToken{}, fmt.Errorf("%w: admin page token query binding mismatch", ErrInvalidArgument)
	}
	return token, nil
}
