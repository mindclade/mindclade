...
func TestOperationAcceptanceCommitsAuditAndOutboxAtomically(t *testing.T) {
	...
	operation, replay, err := operations.Create(policies.DenyByDefault{}, repository, operations.CreateCommand{
		Principal: principal, IdempotencyKey: "key-1", Operation: operations.Operation{ID: "operation-1", TenantID: "tenant-a", JobID: "job-1", RequestHash: "sha256:request"},
	})
	if err != nil || replay || operation.Status != "PENDING" {
		t.Fatalf("unexpected operation result: %#v replay=%v err=%v", operation, replay, err)
	}
	if len(repository.AuditEvents()) != 1 || len(repository.OutboxMessages()) != 1 {
...
func TestArtifactFinalizeIsAtomicAndRejectsCorruption(t *testing.T) {
	...
	_, err := artifacts.Finalize(context.Background(), policies.DenyByDefault{}, repository, storage.FilesystemCAS{Root: root}, &memoryCatalog{}, artifacts.FinalizeCommand{Principal: principal, Artifact: artifacts.Artifact{TenantID: "tenant-a", Digest: digest, MediaType: "application/octet-stream", Size: int64(len(body))}, Body: bytes.NewReader(body)})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPostgresKernelJourney(t *testing.T) {
	...
	operation, replay, err := operationsSQL.CreateAtomicallySQL(context, operations.Operation{ID: operationID, TenantID: tenantID, JobID: jobID, RequestHash: requestHash}, "journey-key", "integration-principal")
	if err != nil || replay || operation.Status != "PENDING" {
		t.Fatalf("PostgreSQL operation acceptance failed: operation=%#v replay=%v err=%v", operation, replay, err)
	}
	if _, replay, err = operationsSQL.CreateAtomicallySQL(context, operations.Operation{ID: operationID, TenantID: tenantID, JobID: jobID, RequestHash: requestHash}, "journey-key", "integration-principal"); err != nil || !replay {
	...
	if err := (jobs.SQLRepository{DB: db}).CompleteAttemptSQL(context, tenantID, attemptID, 1, "SUCCEEDED"); !errors.Is(err, jobs.ErrStaleCompletion) {
